  ...
  export const PropertyLocationFields: React.FC<PropertyFormFieldsProps> = ({
    ...
    useEffect(() => {
      const fetchAllLocationData = async () => {
        ...
        if (selectedProvince) {
          // Fetch districts
          const districtRes = await $axios.get(`/api/address/district/${selectedProvince}`);
          setDistrictFields(districtRes.data.data);
          
          if (selectedDistrict) {
            // Fetch municipalities
            const municipalityRes = await $axios.get(`/api/address/municipality/${selectedDistrict}`);
            setMunicipalityFields(municipalityRes.data.data);
            
            if (selectedMunicipality) {
              // Fetch wards
              const wardRes = await $axios.get(`/api/address/ward/${selectedMunicipality}`);
